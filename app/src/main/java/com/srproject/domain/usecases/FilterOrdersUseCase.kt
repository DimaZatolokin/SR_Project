...
class FilterOrdersUseCase(private val repository: Repository) : BaseUseCase() {
    ...
    private lateinit var filteredOrdersLiveData: LiveData<List<Order>>
    private val filteredOrdersObserver = Observer<List<Order>> { orders ->
        launch {
    ...
    override fun cancel() {
        super.cancel()
        if (::filteredOrdersLiveData.isInitialized) {
            filteredOrdersLiveData.removeObserver(filteredOrdersObserver)
        }
    }

    fun filterOrders(notDone: Boolean, notPaid: Boolean, action: (List<OrderUI>) -> Unit) {
        this.action = action
        launch {
            filteredOrdersLiveData = repository.getFilteredOrders(notDone, notPaid)
            withContext(Dispatchers.Main) {
                filteredOrdersLiveData.observeForever(filteredOrdersObserver)
            }
        }
    }
}