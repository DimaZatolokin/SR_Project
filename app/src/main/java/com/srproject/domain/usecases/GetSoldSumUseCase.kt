...
class GetSoldSumUseCase(private val repository: Repository) : BaseUseCase() {
    ...
    fun getSoldSumPerMonth(date: Date, action: (Int) -> Unit) {
        launch {
            val sum = repository.getSoldSumPerPeriod(date.getBeginMonthDate(), date.getEndMonthDate())
            withContext(Dispatchers.Main) {
                action.invoke(sum)
            }
        }
